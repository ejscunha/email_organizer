defmodule EmailOrganizer.SubscriptionManagerTest do
  ...
  describe "subscription_manager" do
    test "subscribe_user_emails/3 sends a cast message to the GenServer" do
      user = build(:user)
      auth_token = "test_auth_token"

      test_pid = self()

      expect(Email, :subscribe_user_emails, fn ^user, ^auth_token ->
        send(test_pid, :subscribe_called)
        :ok
      end)

      SubscriptionManager.subscribe_user_emails(:subscription_manager, user, auth_token)
